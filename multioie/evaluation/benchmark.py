...
class Benchmark:
    ...
    def compare(self, gold, predicted, matchingFunc, output_fn, error_file=None):
        ...
        gold = Benchmark.normalizeDict(gold)
        ...
        for sent, goldExtractions in gold.items():
            ...
            for predictedEx in [x for x in predictedExtractions if (("matched" not in x) or (output_fn not in x["matched"]))]:
                # Add false positives
                y_true.append(0)
                y_scores.append(predictedEx["confidence"])
        ...
        with open(output_fn, 'w') as fout:
            fout.write(f"{non_sent=}\n")
            fout.write(f"{non_match=}\n")
            fout.write(f"{correctTotal=}\n")
            fout.write(f"{unmatchedCount=}\n")
            fout.write("AUC: {}\n Optimal (precision, recall, F1, threshold): {}\n".format(cur_auc, optimal))
